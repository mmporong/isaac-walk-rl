...
- 다음 단계는 correctness timing과 분리된 throughput 계약을 먼저 사전등록한 뒤 GPU env ladder `[8,32,128,256,512,1024]`를 순차 실행하는 것이다. median env-control-steps/s, peak VRAM, GPU utilization, OOM, numeric·physics stability와 반복성을 기록하고 stable maximum을 고른다. 이 단계를 통과해도 matrix 기반 Gate01은 별도 사전등록·실행하며, 그 뒤에만 scratch PPO qualification을 검토한다.
