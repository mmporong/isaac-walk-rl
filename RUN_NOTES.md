...
#### rev26 E019 R0 seed42 300-iteration qualification 기각

- clean source commit `159338ce507b77836862082ba1cfa5bfc31a271d`, source bundle SHA-256 `9f19c5727331e13ef1395c22eed4065ed11ee07c3ad67d7e7569ad3ecfd1fead`에서 `Isaac-G009-Recover-Flat-Go2-R0-Matrix-v0`를 seed `42`, `cuda:0`, headless, scratch로 실행했다. run은 `go2_flat_g009_r0_rev26_qualification_retry03_s42_20260904-1637`이다. 명령 계약은 `1024 env × 24 steps/env × 300 iterations = 7,372,800 transitions`, PPO epoch `5`, mini-batch `4`, 총 optimizer mini-batch update `6,000회`다.
- 학습 프로세스는 exit code `0`으로 300 iteration을 완료했다. wall time은 `1,157.287s`, steps/s 평균 `6,598.83`, 중앙값 `6,669.5`, final mean reward `5.57`, final mean episode length `400`이었다. numeric-invalid는 300개 scalar sample 모두 `0`이었다. GPU peak는 `4,545MiB`, peak utilization `61%`, peak temperature `60°C`, peak power `62.07W`였고 종료 뒤 baseline으로 회복했다.
- 그러나 hard-joint-limit은 300개 sample 중 57개가 nonzero였고 maximum `0.125`, mean `0.009027778034408887`이었다. 사전등록한 qualification은 numeric-invalid와 hard-joint-limit maximum이 모두 정확히 `0`이어야 하므로 policy qualification은 `false`다. 유일하게 실패한 필수 check는 `qualification_training_safety_zero`였으며 held-out seed 평가는 시작하지 않았다.
- [raw training report](reports/runs/go2_flat_g009_r0_rev26_qualification_retry03_s42_20260904-1637.json)의 SHA-256은 `71a3b45129b79f2beaed14fab486423aafdd0f92e860022cf22c2c6242391234`다. checkpoint `model_299.pt`의 SHA-256은 `75b38ac4c8f8b2ed17d73893350c1ca484ca3ef3c0d633273553e13efdf44c95`다. 따라서 이 checkpoint는 진단 입력으로만 사용하며 qualification·복구 성공·포트폴리오 성공 증거로 사용하지 않는다.
- stage가 기각됐으므로 새 카메라 영상, MP4, GIF, PNG를 만들지 않았고 Garden이나 포트폴리오에도 발행하지 않는다. 다음 단계는 학습을 반복하는 것이 아니라 동일 checkpoint에서 joint·pose attribution을 수집하는 read-only 진단이다.

#### rev27 E020 model_299 hard-joint-limit read-only 진단

- rev26의 [training report](reports/runs/go2_flat_g009_r0_rev26_qualification_retry03_s42_20260904-1637.json)와 `model_299.pt`를 immutable input으로 묶고, clean diagnostic commit `ac0aabf24b6ae6ca952a8730bc5b95b97121ff2d`에서 seed `42`, `cuda:0`, headless, `1024 env`, 자세별 `256 env`, `400 steps`, stochastic policy replay를 실행했다. 자세는 `[prone, supine, left_side, right_side]` 네 종류다. wall time은 `62.561s`다.
- policy, optimizer, checkpoint SHA-256은 실행 전후 각각 동일했다. PPO update와 optimizer update는 모두 `0`이며, 공식 qualification·성공률 평가가 아니라 rev26의 안전 위반을 재현하고 위치를 좁히는 진단이다.
- `+0.01rad` margin을 넘은 hard-limit event는 5개 episode/5개 joint sample에서 재현됐다. 전부 prone 자세의 calf joint였으며 `FR 2회`, `RL 2회`, `RR 1회`다. threshold 초과 최댓값은 `FR 0.006445636749267791rad`, `RL 0.002242555618286346rad`, `RR 0.011725416183471893rad`였다. supine, left-side, right-side에서는 이 replay window의 hard-limit event가 없었다.
- 다섯 sample의 policy action은 wrapper 앞에서 범위를 벗어난 경우가 있었고 wrapper 뒤 normalized action은 모두 `±1`로 clip됐다. 그 뒤 action manager가 만든 calf target은 약 `-2.3346~-2.0208rad`로 hard lower bound `-2.722700119rad`보다 안쪽이었다. 따라서 이번 증거는 “target 명령 자체가 hard bound 밖이었다”는 설명과 맞지 않고, prone 접촉 상태에서 동역학적 overshoot가 발생한 쪽을 지지한다. 다만 한 seed의 짧은 replay이므로 일반적인 인과관계나 다른 자세의 절대 안전을 주장하지 않는다.
- [diagnostic report](reports/runs/g009_5_r0_rev27_model299_joint_limit_diagnostic_s42_20260904-1813.json)의 SHA-256은 `ffd373d3937558aa71b5afccc70468aff068a28443f728a94343e488046bc315`다. 진단은 학습 상태를 바꾸지 않았고 qualification은 계속 실패 상태다. 새 미디어와 Garden·포트폴리오 발행은 만들지 않는다.

#### rev28 E021 entropy=0 단일변수 50-iteration safety smoke 기각

- rev27 attribution 뒤 첫 개입은 PPO entropy coefficient만 `0.01 → 0.0`으로 바꾸는 것으로 제한했다. initial noise std `0.5`, action scale `0.70`, EMA alpha `0.2`, soft joint limit factor `0.9`, solver `8/0`, reset·reward·curriculum·seed·환경 수·rollout 길이·acceptance gate는 고정했다. clean source commit `876c4ae0a89760e5bf3d41180af3d64a53cdef49`, source bundle SHA-256 `3440c70d035e6ee7a5d4e67d83ba0c3a5c4ae1d30dc9def3c346b66fcd5dc831`에 결합했다.
- run `go2_flat_g009_r0_rev28_entropy_smoke_retry01_s42_20260904-2015`는 seed `42`, `cuda:0`, headless, scratch, `1024 env × 24 steps/env × 50 iterations = 1,228,800 transitions`를 수행했다. PPO epoch `5`, mini-batch `4`, optimizer mini-batch update는 `1,000회`다. runtime `params/agent.yaml`에서 entropy `0.0`, initial std `0.5`, rollout `24`, epoch `5`, mini-batch `4`, iteration `50`, device `cuda:0`을 다시 읽어 계약과 일치함을 확인했다.
- wall time은 `162.897s`, steps/s 평균 `8,599.98`, 중앙값 `8,685`, GPU peak `4,420MiB`, peak utilization `58%`, peak temperature `54°C`, peak power `57.1W`였다. fatal event는 없었고 자식 프로세스가 종료됐으며 VRAM도 baseline으로 회복했다. numeric-invalid는 50개 sample 모두 `0`이었다.
- hard-joint-limit은 50개 sample 중 4개가 nonzero였고 maximum `0.0833333358168602`, mean `0.005000000149011612`였다. 요구값은 maximum `0`, nonzero sample `0`이므로 smoke는 실패했다. canonical summarizer도 `hard_joint_limit maximum must be zero`로 fail-closed했고 accepted summary를 쓰지 않았다. 따라서 300-iteration full run과 held-out seed 평가는 모두 금지 상태다.
- 같은 seed의 rev26 첫 50 scalar sample과 비교하면 entropy `0.01`은 nonzero `8/50`, maximum `0.0416666679084301`, mean `0.006666666865348816`, latest mean noise std `0.5513023734092712`였다. rev28 entropy `0.0`은 nonzero `4/50`, maximum `0.0833333358168602`, mean `0.005000000149011612`, latest mean noise std `0.47734370827674866`이었다. 탐색 노이즈와 nonzero sample 수는 줄었지만 worst sample은 커졌으므로 entropy 제거만으로 안전 문제가 해결됐다고 볼 수 없다. 한 seed와 한 window의 동시 변화이므로 인과도 확정하지 않는다.
- [raw training report](reports/runs/go2_flat_g009_r0_rev28_entropy_smoke_retry01_s42_20260904-2015.json)의 SHA-256은 `a3178b3e35969b94a234e98a97106c2be9c4a7fa182d0f8a1fee166201e4e49d`, [rejection synthesis](reports/runs/g009_5_r0_rev28_entropy_smoke_rejection_s42_20260904-2015.json)의 SHA-256은 `712d8f1e788c14d03e86211dd7d44813c3467a577f26a9ed8268b9c1b379577f`다. checkpoint `model_49.pt` SHA-256은 `091b1818c323ed509c504dbe52c3ccbf66537b8e655ff53383b76b0765b01482`이며 qualification 입력으로 사용하지 않는다.
- 이 기각 단계도 새 영상·GIF·PNG와 Garden·포트폴리오 발행을 만들지 않는다. 다음 사전등록 후보는 action scale만 `0.70 → 0.65`로 좁히는 rev29 50-iteration safety smoke다. entropy `0.0`, initial std `0.5`, EMA `0.2`와 나머지 physics·reset·reward·curriculum·seed·budget·zero-event gate는 고정한다. rev29가 zero-event gate를 통과하기 전에는 300-iteration 학습을 열지 않는다.

#### 다음 세션 재개 지점: rev29 action-scale safety smoke

1. `configs/g009_r0_rev29_action_scale_smoke.json`에 단일 변경 `action_scale=0.65`와 frozen 변수, `50 iterations`, zero-event acceptance gate를 사전등록한다.
2. historical rev28 validator·summarizer를 덮어쓰지 않고 rev29 전용 validator·summarizer와 회귀 테스트를 추가한다. runtime `params/agent.yaml` 및 live action config readback에서 entropy `0.0`, initial std `0.5`, action scale `0.65`, EMA `0.2`를 fail-closed로 검증한다.
...
#### rev29 E022 action-scale=0.65 safety smoke 기각과 다음 재개 지점

- clean source commit `a288e285e2b8bf03ff9c5c1761013afbd4c5c58b`, source bundle SHA-256 `a63a469663c1b1bc553750fe4f3f7d506b3b5eb7a5458667a6a64821093d800f`에서 `go2_flat_g009_r0_rev29_action_scale_smoke_retry01_s42_20260904-2103`을 실행했다. 첫 호출은 PowerShell 배열 인자 바인딩 오류로 Isaac 실행 전에 차단됐고 rollout·PPO·artifact가 없었다. retry01은 같은 clean source와 계약을 PowerShell 내부 splatting으로 전달했다.
- retry01은 seed `42`, `cuda:0`, headless scratch, `1024 env × 24 steps/env × 50 iterations = 1,228,800 transitions`, PPO epoch `5`, mini-batch `4`, optimizer mini-batch update `1,000회`를 완료했다. process exit code는 `0`, wall time은 `160.276s`, 평균/중앙 steps/s는 `8,744.1/8,828.5`, final mean reward는 `-10.31`, final mean episode length는 `400`이었다.
- 실행 시 `params/agent.yaml`에서 entropy `0.0`, initial std `0.5`, rollout `24`, epoch `5`, mini-batch `4`, iteration `50`, device `cuda:0`을 읽었고, `params/env.yaml`에서 action scale `0.65`, rescale-to-limits `true`, EMA `0.2`, soft-limit factor `0.9`, solver `8/0`, max depenetration velocity `1.0m/s`를 읽어 사전등록과 일치함을 확인했다. source snapshot도 실행 전후 동일했다.
- numeric-invalid는 `0/50`, maximum `0`으로 PASS했다. hard-joint-limit은 iteration `19`에서 `0.0416666679084301` 한 건이 발생해 `1/50`, mean `0.000833333358168602`, maximum `0.0416666679084301`이었다. zero-event gate는 maximum과 nonzero count가 모두 `0`이어야 하므로 rev29는 기각됐다. canonical summarizer는 `hard_joint_limit maximum must be zero`로 fail-closed했고 accepted summary를 쓰지 않았다.
- rev28 action scale `0.70`의 `4/50`, maximum `0.0833333358168602`, mean `0.005000000149011612`와 비교하면 빈도는 `-3`, maximum은 `-0.0416666679084301`, mean은 `-0.00416666679084301` 줄었다. mean noise std latest는 rev28 `0.47734370827674866`, rev29 `0.4784904718399048`로 거의 같았다. scale 축소와 위반 감소가 함께 관측됐지만 한 seed·한 window이므로 인과나 일반화된 안전을 주장하지 않는다.
- GPU peak는 `4,417MiB`, peak/mean utilization은 `63%/24.63%`, peak temperature는 `54°C`, peak power는 `56.68W`였다. fatal GPU event는 없었고 자식 프로세스가 모두 종료됐으며 VRAM도 baseline으로 회복했다. checkpoint `model_49.pt` SHA-256은 `55d4c3793858ddd2522bf4bac6e163d0b7faf00c1e76664bcbaf112b990f40ad`, 12D std는 모두 finite이고 평균 `0.47849053144454956`이었다.
- [raw report](reports/runs/go2_flat_g009_r0_rev29_action_scale_smoke_retry01_s42_20260904-2103.json) SHA-256은 `575c38b015f0d04552bd7276524339a6ec40ed1336903c580ca38b401bce852d`, [rejection synthesis](reports/runs/g009_5_r0_rev29_action_scale_smoke_rejection_s42_20260904-2103.json) SHA-256은 `9da6ecd5122c36973637a9b0e45f39c911e1287ae3dcc3487d565833156c7160`이다. 300-iteration 학습·held-out seed·Garden·포트폴리오 발행·새 영상은 계속 금지한다.
- 다음 세션은 같은 설정에서 action scale만 `0.65 → 0.60`으로 낮추는 rev30 50-iteration 후보를 새 사전등록으로 만드는 지점부터 재개한다. 이는 확정값이 아니라 rev29의 개선 방향을 한 단계 더 검증할 후보이며, 동일 zero-event gate를 통과하기 전에는 full run으로 넘어가지 않는다.
