...
| 기호                 | 뜻                            |
| -------------------- | ----------------------------- |
| `ref`, `cmd`         | 목표값 표시, 명령값 표시      |
| `t`, `N`             | 미래까지의 시간, 미래 단계 수 |
| `x_ref,i`, `X_ref`   | i번째 목표, 미래 목표 전체    |
| `p_x,p_y,p_z`        | 몸통 위치의 x·y·z 성분        |
| `p_x,0`              | reference 시작 시 x 위치      |
| `v_x,cmd`            | 명령한 x 방향 속도            |
| `h_nominal`          | 평지의 기본 몸통 높이         |
| `roll,pitch,yaw`     | 몸통 자세의 세 성분           |
| `yaw₀`               | reference 시작 시 yaw         |
| `yaw rate cmd`       | 명령한 yaw 회전 속도          |
| `v_ref`, `omega_ref` | 목표 선속도, 목표 각속도      |
| `Q`                  | reference 오차의 중요도       |
...
1. `미래 위치 목표 = 현재 위치 + 명령 속도·미래 시간`
2. `미래 yaw 목표 = 현재 yaw + yaw 명령 속도·미래 시간`
3. Reference는 목표값이고 `Q`는 그 목표의 중요도라는 차이

**뜻만 이해할 것**

- 위치 목표와 속도 목표를 함께 두는 이유
- 경사면에서는 world 수평이 아니라 support plane 기준 목표가 필요할 수 있다는 점

**지금 외우지 않을 것**

- `X_ref`를 메모리에 배치하는 순서
- 계단과 경사 skill의 구체적인 reference 수치
...
**[이해할 식] 소문자 목표 N개를 모으면 horizon 전체 목표가 된다**

```text
X_ref = [x_ref,1, x_ref,2, ..., x_ref,N]을 세로로 쌓은 미래 목표
```
...
```text
x 위치 목표(t) = 현재 x 위치 + x 방향 명령 속도 · t
```

**[외울 식 2] y 방향도 같은 구조**

```text
y 위치 목표(t) = 현재 y 위치 + y 방향 명령 속도 · t
```
...
```text
yaw 목표(t) = 현재 yaw + yaw 회전 명령 속도 · t
```
...
**[이해할 식] 명령 속도를 3차원 목표로 조립**

```text
선속도 목표 = [x 방향 명령 속도, y 방향 명령 속도, 0]
```

각속도 reference는 yaw 회전 명령만 남긴다.

**[이해할 식] yaw 회전만 명령한 경우의 각속도 목표**

```text
각속도 목표 = [0, 0, yaw 회전 명령 속도]
```
