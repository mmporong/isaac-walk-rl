...
각 문서 첫부분에 그 문서에서 사용하는 모든 핵심 변수를 표로 설명한다. 같은 글자라도 문서에 따라 뜻이 달라질 수 있으므로 변수표를 먼저 읽는다.

| 표시 | 해야 할 일 |
|---|---|
| `[외울 식]` | 식의 글자 배열만 암기하지 말고, 각 항의 물리적 뜻과 함께 자료 없이 다시 쓴다. |
| `[이해할 식]` | 식을 보고 각 항의 역할을 설명할 수 있으면 된다. 통째로 외우지 않는다. |
| `[참고 식]` | 구현하거나 계산할 때 다시 찾아본다. 지금 외우지 않는다. |

본문의 수식은 `[외울 식]`이라고 명시된 것만 외운다. 표시가 없거나 `[이해할 식]`, `[참고 식]`으로 적힌 수식은 구조와 의미를 이해하는 것이 목표다.

특히 아래 글자는 문맥에 따라 뜻이 달라진다.

| 글자 | 문서에 따른 뜻 |
|---|---|
| `B` | 01·02에서는 발 힘의 상태 영향 행렬, 06의 `B(u)`에서는 Bézier 곡선 |
| `u` | 01·02에서는 MPC 입력인 네 발 힘, 06에서는 Bézier 구간 진행 변수 |
| `q` | 01에서는 관절각, 02의 QP 식에서는 선형 비용 벡터 |
| `f` / `F` | 04의 소문자 `f`는 보행 주파수, 01·02의 대문자 `F`는 발 힘 |
