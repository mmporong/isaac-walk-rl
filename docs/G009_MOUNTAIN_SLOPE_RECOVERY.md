...
- 현재 단계: C0·S0 완료, G009-5 R0 rev1~rev16 학습·안전 실패 진단과 rev17~rev23 접촉 관측 계보를 마쳤다. rev23 E016에서 read-only matrix observation adapter의 CPU/GPU `2×2` runtime correctness를 검증했고, rev24 E017은 첫 1024 provenance 실패를 보존한 뒤 clean source에서 fresh `1024 → 2048 env` GPU throughput smoke를 canonical PASS해 stable maximum `2048`을 확정했다. rev25 E018 Matrix Gate01은 raw world-frame authority를 보존하면서 base-frame·body-weight·`tanh` projection `57D`를 붙여 actor `140D`, critic `164D`, solver `8/0` 계약으로 구현과 사전등록을 검토 중이며 GPU 실행 전이다.
- 현재 한계: rev24의 첫 1024 실행은 source-bundle 정렬 오류로 기각했지만 수정 후 clean commit에서 fresh 1024와 2048을 모두 canonical PASS해 이번 ladder의 stable maximum을 `2048 env`로 확정했다. Matrix Gate01과 정식 qualification은 미실행이며 새 영상도 없다. throughput smoke는 policy qualification이나 복구 성공을 뜻하지 않는다. source는 filtered normal contact-force vector이며 total·tangential force, 마찰 효과의 직접 관측, physics ground truth, CPU↔GPU 수치 동등성, 보행·회전·경사·전복 복구 성공을 뜻하지 않는다. `learned_policy_qualified=false`를 유지한다.
...
| `G009-5` | `R0` | 평지 네 전복 자세 RECOVER PPO와 선행 안전·관측 진단 | rev24 E017 fresh 1024/2048 throughput PASS, rev25 Matrix Gate01·정식 qualification 미실행 |
...
### 현재 rev24 actor/critic, action, 성공 gate
...
20a. **R0 rev25 whole-body terrain-contact Matrix Gate01**: raw world-frame `[N,19,3]` authority를 보존하고 base-frame·body-weight·`tanh` projection `57D`를 actor와 critic actor-prefix에 연결한다. actor/critic `140/164D`, ordered body hash, Gate 전용 runtime telemetry, live `8/0`·depenetration/action readback, 정확히 20 optimizer update와 matrix-column Adam moment, numeric/hard-limit zero를 먼저 검증한다.
