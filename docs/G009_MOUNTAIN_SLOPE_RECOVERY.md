...
다음 촬영부터 원본 MP4는 30fps로 보존하고 공개 GIF는 15fps를 목표로 만든다. 12fps 아래로 낮추지 않으며, 텔레메트리 애니메이션은 소수의 정지 화면을 반복하는 대신 중간 프레임을 렌더링한다. 10 MiB 제한을 넘으면 길이, 해상도, 팔레트 순서로 줄인다.
...
- RECOVER action: `EMAJointPositionToLimitsAction`, scale `0.70`, alpha `0.2`, asset soft-limit factor `0.9`
- RECOVER 유효 target 범위: hard joint range의 중앙 `63%`; URDF hard-limit termination tolerance `0.01rad` 유지
...
#### rev21 E014 실제 결과: matrix authority safety gate

rev21은 rev20에서 검증한 terrain-pair contact matrix 후보를 다음 runtime 단계의 안전 판정 입력으로 다룰 수 있는지 확인한 **Python 정적 bounded recursive evidence gate**다. source commit은 `e202ae1d514c7abfe05ce0da130c2db47e9e05f3`, rev21 source bundle SHA-256은 `3589f3852c4365afadb8dbdc871ffb2fe81888155b7e66c274034b5506eaf40e`다.

##### 입력 증거와 검증 범위

입력은 [rev20 final synthesis](../reports/runs/g009_r0_rev20_terrain_contact_matrix_synthesis_2x2_s42.json)와 그 synthesis가 묶은 CPU raw 2회, GPU raw 2회, CPU preflight다. final synthesis SHA-256은 `dcb8f446a212390f94f9ae5ccad97d9e770f9b8f5961f5ffb0c920f8d62580b3`다. 여기에 historical source commit `fb2992965fcfb502a679065eac253a6bdcdf7086`의 Git blob 12개를 다시 읽어 source provenance를 확인했다.

gate는 저장된 증거를 제한된 깊이에서 다시 파싱하고 다음 항목을 독립적으로 대조했다.

1. preregistration, canonical output path와 rev21 source binding
2. rev20 synthesis path·SHA-256·schema·decision·governance
3. raw report 4개와 CPU preflight의 path·SHA-256·execution identity
4. historical source 12개 blob과 runtime/synthesis bundle
5. runtime baseline, matrix shape·body/filter ordering·dtype·device
6. finite 값, direct/buffer storage independence와 `150/150` exact parity
7. same-body overlap, mass continuity, non-foot peak와 joint margin

검증 항목 `17/17`이 PASS했다. [canonical artifact](../reports/runs/g009_r0_rev21_matrix_authority_safety_gate_s42.json)의 SHA-256은 `68a60383d9d49bf009d189498f94e6fe1c03155259932dcff8965caa7d9aa250`다.
...
canonical 생성 전 `--check-only`는 어떤 파일도 쓰지 않고 PASS했다. 생성 후 [full verifier](../scripts/summarize_g009_r0_rev21_matrix_authority_safety_gate.py)는 artifact에 기록된 값만 믿지 않고 raw report 4개, CPU preflight, rev20 synthesis, historical Git blob 12개와 rev21 source commit을 다시 읽어 PASS했다. [runner](../scripts/run_g009_r0_rev21_matrix_authority_safety_gate.py)를 두 번째로 실행했을 때는 exit code `2`, reason `canonical_output_already_exists`로 닫혔고 canonical SHA-256은 그대로였다. 재실행 실패 envelope는 `C:\Users\LIMMM\IsaacLab\logs\visual_evidence\g009\R0\diagnostic\failed_attempts\rev21\g009_5_e014_rev21_matrix_authority_safety_gate_75f18ba89aa74171bc8a26797416865d.json`에만 보존했다.

##### 실행과 재검증

다음 명령은 Windows PowerShell에서 실행한다. canonical artifact가 이미 존재하는 현재 상태에서는 `--check-only`가 no-overwrite 규칙 때문에 exit code `2`를 반환하는 것이 정상이다. 최초 생성 전 실행에서는 read-only PASS였다.
...
##### 포트폴리오에서 말할 수 있는 것과 없는 것

이 결과의 의미는 “CPU/GPU `2×2` contact-matrix 증거를 고정된 source와 연결하고, 다음 observation adapter를 열기 전에 shape·ordering·수치·진단 안전 조건을 재현 가능한 fail-closed gate로 만들었다”는 데 있다. 후보 tensor를 바로 정책 입력으로 연결하지 않고, provenance와 no-overwrite까지 검증했다는 점이 문제 해결 과정의 성과다.

그러나 이 단계는 Isaac/PhysX 재실행 `0`, reward 계산 `0`, rollout `0`, PPO/optimizer update `0`인 정적 검증이다. 보행, 앞뒤 이동, 좌·우회전, 경사 주행, 전복 자가복구를 수행하거나 qualification한 결과가 아니다. `15BW`와 hard joint limit 바깥 `-0.01rad` diagnostic margin도 저장된 rev20 증거를 거르는 연구용 기준이지, 실물 로봇의 안전 한계가 아니다. PhysX tensor를 새로 생성하거나 외부 Isaac Lab 설치본의 현재 byte를 재관찰하지 않았으므로 physics ground truth authority라고 주장하지 않는다.

##### 미디어와 공개 보류

rev21은 실행 동작, 물리 조건, checkpoint를 바꾸지 않았다. 따라서 새 영상을 촬영하지 않고 번호 13의 E013 telemetry GIF·PNG와 로컬 MP4를 입력 증거의 시각 자료로 재사용한다. 정적 gate PASS를 새 로봇 동작이나 자가복구 성공 영상처럼 연출하지 않는다. Garden 공개 글은 read-only runtime adapter와 실제 시뮬레이션 관찰 결과가 나온 뒤 작성하며, 현재 E014만 별도 성공 글로 발행하지 않는다.

##### 다음 단계: read-only matrix observation adapter 사전등록

`preregister_read_only_matrix_observation_adapter`에서 다음 계약을 구현 전에 고정한다.

1. 정책 후보 입력을 XYZ vector로 유지할지 magnitude로 축약할지 구분한다.
2. world/local 좌표계 변환과 body-filter 축의 합산·정렬 순서를 고정한다.
3. normalization, clipping, missing contact의 zero/mask/fail-closed 처리를 고정한다.
4. dtype, device, batch·body·filter·component output shape를 고정한다.
5. adapter가 원본 source tensor를 제자리 수정하지 않는지 storage와 byte hash로 확인한다.
6. 이 단계에서도 reward·policy·PPO 연결을 금지하고 simulator launch와 rollout을 `0`으로 유지한다.
7. rev21 정적 파일·JSON·Git blob 검증은 GPU 이점이 없어 CPU에서 수행했으며 GPU PhysX/PPO 실행으로 표현하지 않는다. adapter 사전등록을 통과한 뒤 실제 재관찰과 이후 학습은 `cuda:0`과 GPU dynamics를 사용한다. env·batch는 VRAM 여유, OOM 부재, physics stability를 통과할 때만 단계적으로 늘리고 GPU utilization, peak VRAM, steps/s를 기록한다. 목표는 점유율 `100%` 자체가 아니라 반복 안정성이 유지되는 최대 throughput이다. CPU는 preflight, summary와 backend 재현 대조에 사용한다.

이 계약이 PASS한 뒤에만 read-only runtime observation을 실제 Isaac/PhysX에서 재관찰한다. 그 결과가 source tensor 불변성, shape/order, finite, missing-contact 규칙을 모두 통과해야 matrix 기반 Gate01의 별도 사전등록으로 넘어간다.

#### rev22 E015 실제 결과: read-only matrix observation adapter 사전등록

rev22는 rev21이 연 observation-adapter 단계에서 **구현 전에 의미와 실패 규칙을 고정한 Python 정적 계약 gate**다. source commit은 `0eca6b7cd4ec74e9cf28762b8e51ae80ddb73766`, rev22 source bundle SHA-256은 `db4cd56fa83bb97bb4d335a6ca1c4181c71314b9cbaedb70eb517877f87d1489`, adapter contract SHA-256은 `05105dbb7cf8646d0c7a5bf667cc9ab78de76131819a9654e43d9465a31d5b43`다.

##### 고정한 관측 계약

입력 source는 `terrain_pair_force_matrix_w [N,19,1,3]`, `torch.float32`, source device 유지다. filter 축을 먼저 합산한 `source.sum(dim=2)`의 world-frame XYZ `[N,19,3]`만 authority-preserving output 후보로 둔다. magnitude `[N,19]`는 이 XYZ에 norm을 적용한 진단값이며 방향과 부호를 잃으므로 정책 입력으로 허용하지 않는다. contact mask는 `magnitude > 1e-6N`의 strict comparison이다.

정상적인 zero contact는 정확한 zero XYZ·zero magnitude·false mask로 보존한다. source가 없거나 shape·dtype·device가 다르거나 nonfinite이면 임의 zero-fill, dtype cast, CPU/GPU fallback 없이 fail-closed한다. normalization, body-weight normalization, clipping, saturation은 모두 `none`이다. adapter 전후 source의 shape·dtype·device·stride·storage identity·version·exact value SHA-256이 같아야 하고, 세 output은 source storage를 alias하면 안 된다.

Isaac Lab 2.1.1의 `ContactSensorData.force_matrix_w` 정의를 다시 대조한 결과 이 값은 **world-frame filtered normal contact force vector**다. total contact force와 tangential friction force를 포함하는 값으로 해석하지 않는다. 그러므로 이 matrix의 변화만으로 마찰 효과를 직접 관측했다고 결론 내릴 수 없다. 향후 네 발의 마찰이 같거나 서로 다른 spatial mosaic를 평가할 때는 발 미끄럼, 접선 상대속도, command tracking error, base lateral drift를 별도 채널로 계측한다.

##### 결과와 재현성

[canonical artifact](../reports/runs/g009_r0_rev22_read_only_matrix_observation_adapter_preregistration_s42.json)의 SHA-256은 `a8e536c7f5b739b983c8d1ce05c701b725b46b8926acc33b457c56ff9fad2343`, execution ID는 `9320566d739e45e285afdae75fed0db7`다. `18/18` 판정이 PASS했다.

```text
outcome = read_only_matrix_observation_adapter_preregistration_passed
...
canonical 생성 전 `--check-only`는 exit code `0`이었고 worktree와 output을 바꾸지 않았다. 생성 후 [full verifier](../scripts/summarize_g009_r0_rev22_read_only_matrix_observation_adapter.py)는 rev21 artifact와 source blob, rev22 source binding, adapter oracle과 claim limit를 다시 읽어 exit code `0`으로 끝났다. [runner](../scripts/run_g009_r0_rev22_read_only_matrix_observation_adapter.py)의 두 번째 실행은 exit code `2`, reason `canonical_output_already_exists`로 닫혔고 canonical SHA-256은 유지됐다. 실패 envelope는 `C:\Users\LIMMM\IsaacLab\logs\visual_evidence\g009\R0\diagnostic\failed_attempts\rev22\g009_5_e015_rev22_read_only_matrix_observation_adapter_e585c8e237254f56892db39c9dc3d225.json`에만 보존했다.

rev22 대상 테스트 `56 passed`, rev20·rev21·Gate01 회귀를 합친 전체 `245 passed`를 확인했다. JSON parse, Python compile, diff whitespace와 placeholder/skip scan도 통과했다. 독립 verifier 최종 판정은 CRITICAL/HIGH/MEDIUM/LOW `0/0/0/0`, `APPROVE`다. 현재 Windows 권한으로 실제 symlink/reparse-point 생성 probe는 수행하지 못했고, 해당 입력을 거부하도록 작성된 코드 경로만 검토했다. `ruff`, `pyright`, `basedpyright`, `mypy`는 호스트에 설치되지 않아 이번 검증에는 포함하지 못했다.

##### 미디어와 Garden 발행 판정

rev22는 simulator, 로봇 동작, 물리 조건, checkpoint를 바꾸지 않은 정적 계약이다. 새 영상·GIF·PNG를 만들면 실제 runtime 성과처럼 오해될 수 있어 생성하지 않았다. 번호 13의 E013 telemetry는 predecessor 문맥으로만 연결하며 rev22 성공 미디어로 부르지 않는다.

별도 Garden 글도 발행하지 않는다. 새 실제 시뮬레이션 결과, 새 물리·학습 결과, 직접 관련된 새 미디어가 없기 때문이다. 다음 runtime adapter가 CPU/GPU `2×2` correctness를 통과하고 해당 실행의 telemetry chart·GIF/PNG가 생겼을 때 rev20 공개 글의 후속 단계로 묶을 가치가 있는지 다시 판정한다.

##### 다음 단계: CPU/GPU runtime correctness 뒤 throughput 확장

1. 사전등록된 계약 그대로 read-only adapter만 구현한다. reward, policy observation 연결, PPO, 물성 변경은 금지한다.
2. `8 env × 150 physics steps`에서 CPU 2회와 `cuda:0` 2회를 독립 실행한다.
3. source 불변성, XYZ·magnitude·mask oracle, shape·dtype·device, finite, repeatability를 네 실행 모두에서 검증한다.
4. correctness PASS 전에는 Gate01과 throughput sweep를 열지 않는다.
5. 별도 throughput 계약을 사전등록한 뒤 GPU env ladder `[8,32,128,256,512,1024]`를 순차 실행한다. 각 rung에서 median env-control-steps/s, peak VRAM, GPU utilization, OOM, numeric·physics stability를 기록한다.
6. 가장 큰 env 수나 GPU 점유율 `100%`를 목표로 삼지 않는다. 반복 안정성을 유지한 rung 중 median throughput이 가장 높은 지점을 stable maximum으로 선택한다.
7. 이 runtime probe도 강화학습은 아니다. 그 다음 matrix 기반 Gate01을 별도로 사전등록·통과한 뒤에만 scratch PPO qualification을 연다.

재검증 명령은 다음과 같다. canonical artifact가 이미 존재하므로 runner의 `--check-only` 재실행은 no-overwrite 규칙에 따라 exit code `2`가 정상이다.
...
#### rev23 E016 실제 결과: read-only matrix observation adapter CPU/GPU runtime

rev23은 rev22에서 고정한 adapter를 구현하고 실제 Isaac runtime source에 연결해 correctness를 확인한 단계다. source commit은 `4f4fb71b5948fa909ea298ff5cce2809f66ef05f`, rev23 source bundle SHA-256은 `60e77825dc1616cd32cc314343c65e4723842359fd0c6d8c93f34a34135407c4`다. friction, mass·inertia, terrain, reset distribution, action, solver, contact/rest offset, reward, policy observation, checkpoint, curriculum은 바꾸지 않았다.

##### 실행 하네스

`Isaac-G009-Recover-Flat-Go2-R0-v0`에서 seed `42`, 8 environment, 150 physics step을 실행했다. `physics_dt=0.005s`, control period `0.02s`, `headless=true`, `render=false`, `fast_shutdown=false`다. canonical 순서는 `cpu.rep1 → cpu.rep2 → CPU preflight → cuda:0.rep1 → cuda:0.rep2 → final synthesis`다. CPU 두 실행의 immutable preflight가 `gpu_stage_authorized`를 반환하기 전에는 GPU AppLauncher를 열지 않았다. 세 번째 실행으로 다수결을 만드는 규칙은 허용하지 않았다.

headless는 GUI 창을 띄우지 않았다는 뜻이다. PhysX와 ContactSensor는 실제로 실행했지만 카메라 렌더링, policy rollout, reward 계산, PPO update는 실행하지 않았다. 150개 표본은 150 physics step이며 150 control step이나 학습 batch가 아니다.

##### runtime adapter와 검증 항목

source는 `contact_forces.data.force_matrix_w [8,19,1,3]`, `torch.float32`, 요청 device 유지다. adapter는 다음 세 값을 계산했다.

```text
world_xyz   = source.sum(dim=2)                  # [8,19,3]
magnitude   = torch.linalg.vector_norm(...,-1)  # [8,19]
contact_mask = magnitude > 1e-6                 # [8,19], torch.bool
```

정상 zero contact는 exact zero XYZ·magnitude와 false mask로 유지한다. normalization, clipping, saturation, zero-fill, dtype cast, CPU/GPU transfer·fallback은 없다. 각 step에서 source shape, dtype, device, stride, storage pointer, storage offset, tensor version, exact-value SHA-256을 adapter 전후에 비교했다. 세 output은 source 및 서로의 storage와 non-alias여야 한다. 150개 step과 대표 step `1/50/100/150`에서 world XYZ, magnitude, mask를 독립 Torch oracle로 재계산했다.

네 실행 모두 source mutation, oracle mismatch, alias violation, source contract violation, zero-contact semantics violation, device violation, nonfinite가 `0`이었다. 이 결과는 adapter가 실제 runtime tensor를 읽고도 source를 바꾸지 않았으며 사전등록된 변환을 수행했다는 뜻이다.

##### CPU preflight와 CPU/GPU 2×2 결과
...
[CPU preflight](../reports/runs/g009_r0_rev23_matrix_observation_adapter_cpu_preflight_2x_s42.json)의 SHA-256은 `f8e1d203a5f2fce679a7ad7b9bf7e244b68bade845431f1582fc34fa443f29a7`, outcome은 `gpu_stage_authorized`다. [최종 CPU/GPU 2×2 synthesis](../reports/runs/g009_r0_rev23_matrix_observation_adapter_synthesis_2x2_s42.json)의 SHA-256은 `a28120dc697625452beed9f9ad160f4acc94da0178ca598816215796caf7ef25`, input count는 `4`다.

CPU 내부 두 반복과 `cuda:0` 내부 두 반복은 각각 exact field와 허용오차 대상 numeric field에서 `repeatable=true`다. 이 판정은 CPU와 GPU 사이의 수치 동등성 gate가 아니다. 장치 간 참고 차이는 max magnitude `1.216064453125N`(`0.0877%`), magnitude integral `7.445416584014893N·s`(`6.6473%`), zero source vector `23`(`0.1245%`)이다. 현재 계약은 이 차이를 PASS/FAIL로 판정하지 않으므로 backend equivalence를 주장하지 않는다.

```text
outcome = read_only_matrix_observation_adapter_runtime_2x2_validated
cpu.repeatable = true
cuda:0.repeatable = true
...
##### Windows 종료 lifecycle 제한

첫 lifecycle 점검은 기본 fast shutdown 뒤 canonical report 발행 경로에 도달하지 못했으므로 정식 반복에 포함하지 않았다. 정식 네 실행은 `fast_shutdown=false`로 `SimulationApp.close()` 뒤 canonical 발행까지 수행했다. Windows 종료 과정에서 Isaac Sim access-violation 경고가 출력됐지만 프로세스 exit code는 `0`이었다.

exit code만으로 PASS하지 않았다. canonical JSON 존재, strict full validator PASS, 150개 sample 완전성, source commit·bundle·입력 SHA binding, canonical no-overwrite, 같은 디렉터리 temporary file `0`을 함께 확인했다. 이 검증은 네 canonical artifact가 완전하게 발행됐다는 근거다. 종료 경고가 다른 Isaac Sim 작업이나 모든 Windows 환경에서 무해하다는 뜻은 아니다.

##### 주장 가능한 것과 없는 것

포트폴리오에서 주장할 수 있는 성과는 “GPU callback이 제공하지 못한 접촉쌍 관측을 filtered matrix 후보로 전환하고, 정적 의미 계약을 거쳐 actual CPU/GPU runtime에서 source 불변성과 변환 correctness를 반복 검증한 fail-closed 관측 파이프라인”이다. 후보 tensor를 바로 policy에 연결하지 않고 source provenance, device 보존, oracle, alias, no-overwrite와 종료 후 발행까지 분리해 검증했다.

source는 world-frame filtered **normal contact-force vector**다. total contact force, tangential friction force 또는 friction 효과를 직접 관측했다고 주장하지 않는다. `physics_ground_truth_authority=false`다. 보행, 앞뒤 이동, 좌·우회전, 경사 주행, 전복 자가복구를 수행하거나 qualification하지 않았다. reward, rollout 학습, PPO·optimizer update는 모두 `0`이며 `learned=false`다.

##### 번호 14 미디어와 Garden 판정

번호 `14.01`은 CPU preflight, `14.02`는 CPU/GPU final을 canonical JSON에서 재구성한 telemetry animation이다. simulation camera footage나 새로운 로봇 운동 영상이 아니다. 모든 프레임에 `TELEMETRY ANIMATION / NOT CAMERA FOOTAGE / DIAGNOSTIC ONLY / NO PPO / NOT QUALIFIED`를 표시한다.

![G009-5 rev23 14.01 CPU matrix adapter telemetry](media/g009/R0/diagnostic/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry.gif)

![G009-5 rev23 14.02 final matrix adapter telemetry](media/g009/R0/diagnostic/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry.gif)

두 GIF와 대표 PNG는 `1280×720`이고 GIF는 8 frame, `5.6s`다. [14.01 visual summary](../reports/runs/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry_visual_summary.json)와 [sidecar](../reports/runs/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry_visual_evidence.json), [14.02 visual summary](../reports/runs/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry_visual_summary.json)와 [sidecar](../reports/runs/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry_visual_evidence.json)가 입력 artifact와 공개 파생물의 provenance를 결합한다.

H.264 `1280×720`, 30fps, 168 frame, `5.6s` MP4는 Git에 추적하지 않는다. `14.01`은 `C:\Users\LIMMM\IsaacLab\logs\visual_evidence\g009\R0\diagnostic\g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry_s42.mp4`, SHA-256 `42fe4696ffd7fc4d719a84b2c72a94de8321ef6d270295e2979b7aaae652beb6`다. `14.02`는 같은 디렉터리의 `g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry_s42.mp4`, SHA-256 `7482a069275516e34fb82fa85904637f068ab45903ec8e22480d31a76a7f07db`다. rev23은 실제 runtime correctness와 직접 대응하는 telemetry를 갖추므로 rev20 공개 글의 후속 진단으로 검토할 수 있다. 다만 걷기나 복구 성공 글로 분류하지 않는다.

##### rev23 당시 다음 단계: throughput ladder 뒤 matrix Gate01

correctness hash·snapshot 계측 시간과 throughput timing을 분리한 계약을 먼저 사전등록한다. GPU env ladder `[8,32,128,256,512,1024]`를 순차 실행하면서 median env-control-steps/s, peak VRAM, GPU utilization, OOM, numeric·physics stability와 반복성을 기록한다. 가장 큰 env 수나 GPU 점유율 `100%`가 아니라 반복 안정성을 유지한 rung 중 median throughput이 가장 높은 지점을 stable maximum으로 선택한다.

throughput PASS가 matrix Gate01을 자동 승인하지 않는다. matrix 기반 Gate01을 별도 사전등록·실행하고 안전 gate를 통과한 뒤에만 scratch PPO qualification을 연다.

#### rev24 E017 실행 직전 체크포인트: 공식 GPU throughput smoke

rev23에서 적어 둔 `[8,32,128,256,512,1024]` 계획은 실행하지 않았다. 기존 G008/G009에서 `1024 env` 실행 경로를 이미 여러 번 사용했고, 이번 목적은 adapter correctness 재검증이 아니라 active 계약의 공식 benchmark throughput 확인이다. rev24는 중간 rung을 늘리지 않고 `1024 env`를 먼저 실행한 뒤 모든 gate를 통과한 경우에만 `2048 env`로 확장하도록 범위를 줄였다.

active 계약은 `g009_r0_recover_rev24`, SHA-256 `64eb108bb736d9ba8c1727c3a56ddc3fefaafaba25a98f93c1f8505704c5dd91`다. solver position/velocity `8/0`, max depenetration `1.0m/s`, action scale `0.70`, EMA alpha `0.2`, PPO initial noise `0.5`를 사용한다. rev15의 position `16` 계약은 [`g009_r0_rev15.json`](../configs/g009_r0_rev15.json)에 historical snapshot으로만 남겼다.

두 rung은 seed `42`, headless scratch PPO, `5 iterations`, `24 steps/env`, learning epoch `5`, mini-batch `4`다. 1024 rung은 `122,880 transitions`, 2048 rung은 `245,760 transitions`이며 각 rung의 optimizer mini-batch update는 `100회`다. PPO update는 실제로 실행하지만 성능 평가에 필요한 길이가 아니므로 policy qualification과 recovery success claim은 모두 false다.
...
구현 커밋 `134b6ee31c5ef220048eaaf632f7872a86122258`에서는 source와 판정기만 검증했다. 이후 commit `1c0b35c2f13b77620d57ad62175ddb87f68bf828`의 첫 1024 diagnostic은 아래 provenance gate에서 기각했다. ordinal 정렬 수정이 반영된 clean commit `0437e3766e6ff50a6b05a788a6cc7872ee582b89`에서는 새 실행 ID로 1024와 2048을 다시 수행해 둘 다 canonical PASS했고 final synthesis가 `stable_max_envs=2048`을 확정했다. 번호 `15.01/15.02` 미디어는 아직 없으므로 Garden 발행 판단은 보류한다. 실제 수치와 hash는 [rev24 GPU throughput 실행 인계](G009_REV24_GPU_THROUGHPUT_CHECKPOINT.md)에 기록했다.
...
##### rev24 E017 수정 후 canonical 1024/2048 결과

- clean commit `0437e3766e6ff50a6b05a788a6cc7872ee582b89`에서 fresh 1024와 2048을 순차 실행했고 두 report의 source bundle SHA-256은 `5b244f87797754926c30dbc64a30fad9d7220859f160cfef2571c60cc25aaec0`으로 같았다.
- 1024 report SHA-256은 `5f39c701bbfd889c2a44f470abcf1d4e4632398e34fd9d9e70406cc7da51fb50`, checkpoint SHA-256은 `0d745dca05a97dd1849b584a0dae100642990afaf07432f416910507c41b67be`다. 2048 report SHA-256은 `27da732b114dc2c6432926814777d4335e5df4309acaf760af45288abb1ca8e9`, checkpoint SHA-256은 `fce57b96b0c3b0ff50e85cae273b8bc11d91c6f63a6417b69f67e91609a40e41`다.
- final synthesis SHA-256 `15281e134159974525fc53050e186e8e16d79108f9a068fe717d8ea26b805358`가 두 rung의 canonical PASS와 `stable_max_envs=2048`을 기록한다. 이는 이번 `[1024,2048]` ladder의 상한이며 GPU 절대 최대나 recovery 성능 수치가 아니다.

#### rev25 E018 whole-body terrain-contact Matrix Gate01 결과

E018의 목적은 `force_matrix_w [N,19,1,3]`에서 filter 축을 합친 raw world-frame `[N,19,3]` authority를 보존하면서, policy에 들어가는 값만 base frame으로 회전하고 nominal body weight `147.33639N`로 정규화한 뒤 elementwise `tanh`로 제한하는 것이다. 19-body × XYZ projection은 `57D`다. actor는 기존 `83D` 뒤에 이를 붙인 `140D`, critic은 같은 uncorrupted actor prefix 뒤에 privileged suffix `24D`를 붙인 `164D`다.

실행 계보를 성공 결과와 섞지 않았다.

| 실행 | source commit | 실제 결과 | canonical 판정 |
| --- | --- | --- | --- |
| base | `818791557dd2696ae68114a792de41e805a887b8` | bootstrap이 `AppLauncher` 전에 Isaac 의존 모듈을 import해 `isaacsim.core`를 찾지 못함. exit `1`, PPO 미실행 | 기각 |
| retry01 | `b0f3fd7097ebec36997cb6c527e76a7455f2b24a` | wrapper/PPO `1 iteration`과 checkpoint 성공, safety scalar `0/0`. simulator 종료 뒤 telemetry를 읽어 matrix telemetry 누락 | 기각 |
| retry02 | `086fa82106ffed5de86ff9e9de3e24d44dc3e593` | telemetry callback을 `SimulationApp.close()` 전에 실행. wrapper·source·runtime·checkpoint·optimizer·safety gate 통과 | `27/27 PASS` |

- [base pre-App import 기각 report](../reports/runs/g009_r0_rev25_matrix_gate01_training_s42_rejected_pre_app_import.json)의 SHA-256은 `7b2e3b4b563b189cbaf27c1ff1e97d71854b3aad09765460712869b2e691cbed`다. 이 시도에는 rollout, PPO update, TensorBoard, checkpoint가 없다.
- [retry01 missing-telemetry 기각 report](../reports/runs/g009_r0_rev25_matrix_gate01_training_retry01_s42_rejected_missing_telemetry.json)의 SHA-256은 `449209f243c03629cfd4fe39bd03bb58d9b3c4b70d30f7d06019e93880e47a14`다. report 안의 `passed=true`는 wrapper run-health 판정일 뿐 E018 canonical PASS가 아니다.
- retry02는 seed `42`, headless scratch, `1024 env × 24 steps × 1 iteration`, PPO epoch `5`, mini-batch `4`, optimizer mini-batch update `20회`다. [training report](../reports/runs/g009_r0_rev25_matrix_gate01_training_retry02_s42.json) SHA-256은 `690d1640e0fe2645fc8e7e1a10459184abaf01296427d40198ffaf653d3ccf96`, 15개 source bundle SHA-256은 `2003b07981a7a2fdadb72a7752a374bb0deee75dbee2c1dba04578d56982f66a`다. source path manifest SHA-256은 `530f2371633a699e56201d7e4d02061913606bb0139094d1901f402ecd562200`, 27개 gate-key manifest SHA-256은 `d1769b57db7df4909f50d0d853a3451096ab0af484c36b1e04b022f73c8056bd`다.
- runtime telemetry는 56회 호출에서 source/output finite와 source 불변을 모두 통과했다. positive magnitude `168,456`, nonzero output `505,368`, 최대 contact magnitude `1,966.054931640625N`, bounded output 범위 `[-0.9213587045669556, 1.0]`, 최대 variance `0.007228894159197807`다. source/output shape은 `1024×19×1×3 → 1024×57`, dtype/device는 `torch.float32 / cuda:0`다. ordered 19-body hash는 `2df7038571d25fe75680727bb2c9c9e87567d8946aad77d414a41a1b61d24436`이다.
- live physics readback은 1,024 articulation 모두 solver position/velocity `8/0`, 1,024×19 body 모두 max depenetration velocity `1.0m/s`, action scale `0.70`, EMA alpha `0.2`다. checkpoint first layer는 actor `[512,140]`, critic `[512,164]`이고, Adam step `20`에서 actor matrix columns `83:140`의 optimizer moment가 nonzero이며 L2는 `0.002019342267885804`다. checkpoint SHA-256은 `8096d5ecc9dd54d4c48f76bc7e51f5aee073b6eac428bcedb51347561ed2ef9e`다.
- wrapper는 exit code `0`, wall time `31.154s`, `4,525 steps/s`, final mean reward `-0.48`, final mean episode length `12.92`를 기록했다. numeric-invalid와 hard-joint-limit maximum은 `0/0`이다. GPU peak는 `4,430/12,288MiB`, baseline 대비 delta `2,950MiB`, peak/mean utilization `27%/7.93%`, peak temperature `53°C`, peak power `43.51W`였고 측정 종료 뒤 baseline `1,480MiB`로 회복했다.
- [canonical E018 synthesis](../reports/runs/g009_r0_rev25_matrix_gate01_retry02_s42.json) SHA-256은 `e81a71dcc3fc4b607ab60bae1c4d3dc1683d15616f312e0aedf24438b37da67d`이며 outcome은 `matrix_gate01_passed`다. 정확한 주장 경계는 `policy qualification=not_run`, `recovery success=not_measured`다.

이번 단계에서 새 MP4·GIF·PNG는 만들지 않았다. 따라서 Garden·포트폴리오 production 발행도 보류한다. E018의 `.item()` 기반 finite·provenance telemetry는 1-iteration Gate 전용이며 production 학습에서는 `collect_gate_telemetry=false`로 비활성화한다. 다음 단계는 Hydra override·resume 없는 seed `42`, headless scratch `1024 env × 24 steps × 300 iterations` R0 qualification이다. 네 자세 각각 성공률 `≥80%`, 중앙 복구시간 `≤4s`, safety termination `0`을 검증하기 전에는 recovery policy를 승인하지 않는다.
...
### 현재 rev25 actor/critic, action, 성공 gate

- actor `140D`: 기존 `P-RECOVER-83`과 whole-body terrain-contact matrix projection `57D`
- critic `164D`: actor prefix `140D`와 simulation-only privileged suffix `24D`
- action: 12관절 position target, scale `0.70`, EMA alpha `0.2`, soft-limit factor `0.9`; hard joint range 중앙 `63%`
...
17. **R0 rev21 matrix authority safety gate — E014 정적 검증 완료**: rev20 synthesis, CPU/GPU raw 각 2회, CPU preflight와 historical source blob 12개를 bounded recursive gate로 재검증했다. `17/17` PASS와 no-overwrite를 확인했지만 Isaac/PhysX, reward, rollout, PPO는 실행하지 않았다.
18. **R0 rev22 read-only matrix observation adapter — E015 정적 사전등록 완료**: world XYZ authority output, 진단용 magnitude, strict mask, missing-contact fail-closed, source 불변성과 non-alias를 고정했다. `18/18` PASS와 no-overwrite를 확인했지만 adapter 구현, runtime 관찰, policy 연결, PPO는 실행하지 않았다. 다음은 CPU/GPU `2×2` runtime correctness probe다.
19. **R0 rev23 read-only matrix observation adapter runtime — E016 correctness 완료**: actual `force_matrix_w`를 CPU와 `cuda:0`에서 각각 두 번 adapter에 통과시켰다. 장치별 `2/2` 반복에서 source 불변성, XYZ·magnitude·mask oracle, non-alias, device 보존을 통과했다. reward·policy·PPO는 연결하지 않았고 다음은 GPU throughput ladder 사전등록이다.
20. **R0 rev24 GPU throughput smoke, E017 완료**: active `8/0` 계약과 공식 benchmark에서 fresh `1024/2048 env × 5 iterations`를 canonical PASS했고 이번 ladder의 stable maximum을 `2048 env`로 확정했다. 이 결과는 matrix Gate01이나 recovery policy를 자동 승인하지 않는다.
20a. **R0 rev25 whole-body terrain-contact Matrix Gate01 — E018 완료**: pre-App import 실패와 wrapper/PPO 성공 뒤 telemetry 누락 실패를 각각 기각 계보로 보존했다. lifecycle을 before-close callback으로 수정한 clean `086fa82` retry02에서 raw world-frame authority, base-frame·body-weight·`tanh` projection `57D`, actor/critic `140/164D`, ordered body hash, live `8/0`·depenetration/action readback, 20 optimizer update와 matrix-column Adam moment, numeric/hard-limit zero를 검증해 사전등록 gate `27/27`을 통과했다. `policy qualification=not_run`, `recovery success=not_measured`다.
21. **R0 qualification**: 별도 Matrix Gate01을 포함한 모든 안전 gate를 통과한 revision에서 Hydra override·resume 없이 `1,024 env × 24 steps × 300 iterations`, seed 42를 다시 scratch로 실행한다. Gate01의 GPU-sync telemetry 경로는 production에 승격하지 않는다. 네 자세 각각 성공률 `≥80%`, 중앙 복구시간 `≤4s`, safety termination `0`을 통과해야 checkpoint를 승인한다.
22. **GATE-R1 freeze**: S0 nominal height, WALK torque·power, R0 RECOVER power·충격 proxy를 calibration하고 별도 verifier가 동결한다.
23. **S1-low WALK**: `5/10°` contour-left/right를 G008 WALK parent에서 seed별 독립 lineage로 학습한다.
24. **S1-high WALK**: `15/20°`를 순차적으로 연다. `25°`는 stress로 유지한다.
25. **D0A/D0B/D0C**: G006 exact 회귀, G009 0도 transfer, 통과한 경사별 delta-velocity를 분리한다.
26. **D1 external wrench**: 힘·시간·충격량 pulse를 mild에서 strong 순서로 추가한다.
27. **S2 residual height**: nominal friction에서 base slope에 G008 도로 residual만 더한다.
28. **S3-controlled**: 발별 비대칭 마찰을 통제한다.
29. **S3-spatial**: 비주기 공간 마찰 mosaic로 옮긴다.
30. **F0A와 R0B**: 실제 WALK 낙상 snapshot을 수집하고 curated/replay `50/50` reset으로 RECOVER를 다시 학습한다.
31. **R1/R2**: 낮은 경사, 높은 경사 self-righting을 차례로 연다.
32. **R3-controlled/R3-spatial**: controlled 마찰 뒤 spatial 마찰 복구를 연다.
33. **D2**: 외란이 만든 live fall을 RECOVER로 넘겨 `push -> fall -> recover -> stand -> command resume`를 평가한다.
34. **F0B-TV와 R4**: training/validation natural-fall inventory로 최종 bridge를 학습한다.
35. **I0**: 세 WALK/RECOVER seed pair의 live·snapshot validation을 통과한 뒤 checkpoint, gate, trigger, component SHA를 동결한다.
36. **F0B-FINAL과 I1**: sealed final-heldout을 처음 열어 한 번만 평가한다.
37. **D3**: `25°`, physical-limit friction, strong wrench를 결합한 stress 평가를 수행한다.
38. **M1 link-mass**: G009 final-heldout을 고정한 뒤 hip, thigh, calf, foot 질량·관성을 한 그룹씩 바꾸는 별도 goal로 연다.
...
15. terrain-pair matrix 후보를 historical Git blob과 raw evidence까지 bounded recursive 검증하는 정적 safety gate로 고정하고, canonical artifact를 재실행으로 덮어쓰지 않는 provenance 경계를 만들었다.
16. normal contact-force matrix를 total·tangential friction force로 과장하지 않고 world XYZ authority, magnitude diagnostic, mask, missing-contact와 source 불변성 계약으로 분해해 runtime 구현 전에 고정했다.
...
- [R0 rev21 E014 matrix authority safety gate](../reports/runs/g009_r0_rev21_matrix_authority_safety_gate_s42.json)
- [R0 rev21 E014 preregistration](../configs/g009_r0_rev21_matrix_authority_safety_gate.json), [runner](../scripts/run_g009_r0_rev21_matrix_authority_safety_gate.py), [full verifier](../scripts/summarize_g009_r0_rev21_matrix_authority_safety_gate.py)
- [R0 rev22 E015 read-only matrix observation adapter artifact](../reports/runs/g009_r0_rev22_read_only_matrix_observation_adapter_preregistration_s42.json)
- [R0 rev22 E015 preregistration](../configs/g009_r0_rev22_read_only_matrix_observation_adapter.json), [runner](../scripts/run_g009_r0_rev22_read_only_matrix_observation_adapter.py), [full verifier](../scripts/summarize_g009_r0_rev22_read_only_matrix_observation_adapter.py)
- [R0 rev23 E016 CPU rep01](../reports/runs/g009_r0_rev23_matrix_observation_adapter_cpu_rep01_s42.json), [CPU rep02](../reports/runs/g009_r0_rev23_matrix_observation_adapter_cpu_rep02_s42.json)
- [R0 rev23 E016 GPU rep01](../reports/runs/g009_r0_rev23_matrix_observation_adapter_gpu_rep01_s42.json), [GPU rep02](../reports/runs/g009_r0_rev23_matrix_observation_adapter_gpu_rep02_s42.json)
- [R0 rev23 E016 CPU preflight](../reports/runs/g009_r0_rev23_matrix_observation_adapter_cpu_preflight_2x_s42.json), [CPU/GPU 2×2 final synthesis](../reports/runs/g009_r0_rev23_matrix_observation_adapter_synthesis_2x2_s42.json)
- [R0 rev23 E016 runtime contract](../configs/g009_r0_rev23_read_only_matrix_observation_adapter_runtime.json), [adapter](../src/isaac_walk_g009/matrix_observation_adapter.py), [probe](../scripts/probe_g009_r0_rev23_matrix_observation_adapter_runtime.py), [synthesis](../scripts/summarize_g009_r0_rev23_matrix_observation_adapter_runtime.py)
- [R0 rev23 E016 14.01 visual summary](../reports/runs/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry_visual_summary.json), [sidecar](../reports/runs/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry_visual_evidence.json)
- [R0 rev23 E016 14.02 visual summary](../reports/runs/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry_visual_summary.json), [sidecar](../reports/runs/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry_visual_evidence.json)
- [R0 rev23 E016 14.01 공개 PNG](media/g009/R0/diagnostic/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry.png), [공개 GIF](media/g009/R0/diagnostic/g009_5_r0_diag_rev23_14_01_cpu_matrix_adapter_telemetry.gif)
- [R0 rev23 E016 14.02 공개 PNG](media/g009/R0/diagnostic/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry.png), [공개 GIF](media/g009/R0/diagnostic/g009_5_r0_diag_rev23_14_02_final_matrix_adapter_telemetry.gif)
- [R0 rev24 active 실행 계약](../configs/g009_r0.json), [rev15 historical snapshot](../configs/g009_r0_rev15.json)
- [R0 rev24 E017 GPU throughput 사전등록](../configs/g009_r0_rev24_gpu_throughput.json)
- [R0 rev24 공식 benchmark wrapper](../scripts/bootstrap_benchmark_g009.py), [strict synthesis](../scripts/summarize_g009_r0_rev24_gpu_throughput.py)
- [R0 rev24 GPU throughput 실행 인계](G009_REV24_GPU_THROUGHPUT_CHECKPOINT.md)
