...
| G009 R0 rev15 position solver 진단 | rev12 `8/0`, max depenetration `1.0m/s`에서 position iteration만 `8→16`; CPU·GPU 각각 새 프로세스 `3회 × 8 env × 150 step`, live solver·152 rigid-body readback, force·CPU separation·numeric/hard safety strict gate | 증거 합성 PASS·후보 REJECT. CPU force `13.2482814789 BW`, separation `-0.00935308635m`는 `3/3` PASS. GPU force `16.7882747650 BW`는 `15 BW`를 `11.92%` 초과해 `3/3` FAIL. Gate01·Gate10·PPO·qualification 미실행, learned=false. 06 실제 CUDA off-screen camera와 07 telemetry 공개 |
