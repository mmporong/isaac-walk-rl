#!/usr/bin/env python3
"""Record the rejected rev15 GPU right-side/hold cell as local-only footage."""
...
EXPECTED_COMMIT = "bc999d504e226011ff3d83e68a416b9049b406cb"
...
EXPECTED_NONFOOT_PEAK_BW = 16.78827476501465
EXPECTED_NONFOOT_STEP = 129
EXPECTED_NONFOOT_TIME_S = 0.645
EXPECTED_NONFOOT_BODY = "base"
...
def write_failure_manifest(
    ...
    write_json_new(
        manifest,
        {
            "schema_version": "g009.r0.rev15.camera_capture_failure.v1",
            "status": "failed",
            "phase": phase,
...
def validate_current_capture_binding() -> dict[str, Any]:
    ...
    require(
        ...
        "current capture source bundle drifted from rev15",
    )
    require(
    ...
    require(
        ...
        "current capture contract drifted from rev15",
    )
...
def validate_runtime_report(path: Path) -> dict[str, Any]:
    ...
    require(
        ...
        "bound rev15 runtime report hash mismatch",
    )
    report = read_json(path)
    source = report.get("source_bundle", {})
    require(
        report.get("schema_version") == 3 and report.get("passed") is False,
        "rev15 GPU probe must remain rejected",
    )
    require(
        report.get("run_health", {}).get("passed") is True
        and report.get("runtime_contract", {}).get("passed") is False
        and report.get("progression_gate", {}).get("passed") is False
        and report.get("progression_gate", {}).get("status")
        == "runtime_contract_failed",
        "GPU runtime/progression rejection mismatch",
    )
    checks = report.get("checks")
    require(isinstance(checks, dict), "runtime checks missing")
    ...
    require(
        report.get("headless") is True and report.get("device") == "cuda:0",
    ...
    require(
        crosscheck.get("authority_device") == "cpu"
    ...
    require(
        cell.get("min_contact_separation_m") is None
        and cell.get("min_contact_separation_provenance") is None,
        "GPU separation must remain unavailable",
    )
    require(
        cell.get("max_nonfoot_force_bodyweights") == EXPECTED_NONFOOT_PEAK_BW,
        "nonfoot force peak mismatch",
    )
    peak_time_s = cell.get("max_nonfoot_force_time_s")
    require(
        type(peak_time_s) in (int, float),
        "nonfoot force time must be numeric",
    )
    assert isinstance(peak_time_s, (int, float)) and not isinstance(peak_time_s, bool)
    require(math.isfinite(float(peak_time_s)), "nonfoot force time must be finite")
    require(
...
def blocking_cell_payload(cell: Mapping[str, Any]) -> dict[str, Any]:
    """Return the exact cross-script binding consumed by the camera media builder."""

    return {
        "env_index": SOURCE_ENV_INDEX,
        "pose_id": POSE_ID,
        "action_mode": ACTION_MODE,
        "min_contact_separation_m": None,
        ...
        "max_nonfoot_force_time_s": cell["max_nonfoot_force_time_s"],
    }
...
def capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(
        ...
        "capture requires a clean descendant of the bound rev15 commit",
    )
    ...
    env_cfg.validate()
    require(
        float(env_cfg.scene.robot.spawn.rigid_props.max_depenetration_velocity) == 1.0,
        "rev15 max depenetration velocity drifted",
    )
    require(
        ...
        "rev15 physics/control timing drifted",
    )
    ...
    live_solver_readback = articulation_solver_iteration_readback(
        stage,
    ...
    live_readback_checks = {
        **articulation_solver_iteration_checks(
            live_solver_readback,
    ...
    require(
        ...
        "live rev15 physics/control timing drifted",
    )
    ...
    try:
        ...
        action_term = recorded.unwrapped.action_manager.get_term("joint_pos")
        soft_limits = robot.data.soft_joint_pos_limits.detach().clone()
        hold = reset_pose_hold_action_diagnostics(
        ...
        require(
            bool((~hold["saturated_mask"]).all().item()),
            "reset-pose hold action must remain unsaturated",
        )
        actions = torch.zeros(
    ...
    cell = runtime["cell"]
    result = {
        "schema_version": "g009.r0.rev15.camera_capture.v1",
        ...
        "live_physics_readback": {
            ...
            "articulation_solver_iterations": live_solver_readback,
        ...
        "action_generation": {
            "mode": "reset_pose_hold",
            "all_actions_exact_zero": False,
            "unsaturated": True,
            "source": "inverse-mapped reset joint positions using live soft limits",
        },
        "original_rev15_report_binding": {
            "path": portable_path(args.runtime_report),
            "sha256": runtime["sha256"],
            "execution_id": EXPECTED_EXECUTION_ID,
            "blocking_cell": blocking_cell_payload(cell),
        },
        "evidence_scope": "condition-matched GPU visual playback; force is bound to the runtime JSON and is not measured from pixels",
        "completed_stages": {
            "cpu_runtime_3x": True,
            "gpu_runtime_3x": True,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    require(
        bool(args.headless) and getattr(args, "device", "cpu") == "cuda:0",
        "capture requires --headless --device cuda:0",
    )
